...
function generateFixCandidates(code) {
  ...
  // Candidate 1: Targeted shape & decision node sanitization
  const lines1 = [];
  for (let l of lines) {
    ...
    if (diagType === 'flowchart' || firstLine.includes('graph')) {
      ...
      // Quote & Sanitize Decision Nodes NodeID{Text} -> NodeID{"Clean Text"}
      const mDec = line.match(/^(\s*)([A-Za-z0-9_\-\.]+)\s*\{([^"\n\}]+)\}(:::\w+)?$/);
      if (mDec) {
        const indent = mDec[1];
        let nid = mDec[2];
        let raw = mDec[3].trim();
