...
<ResearchNote
  title="向量库:HNSW/IVF 与存算分离"
  source="Milvus Engineering Blog"
  href="https://milvus.io/blog/"
  insight="Milvus 用存算分离架构承载海量向量检索,索引层可选 HNSW(高召回低延迟)或 IVF(省内存),在召回率、延迟与内存之间按场景取舍。"
  application="架构启示:ANN 检索没有万能索引——HNSW 与 IVF 是「延迟/内存」光谱的两端;向量库的价值在于把索引选型与存算扩展解耦。"
/>
