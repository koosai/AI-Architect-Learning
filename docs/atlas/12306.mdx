...
<ResearchNote
  title="内存网格削峰:把库存扣减搬出数据库"
  source="Apache Geode 官方文档"
  href="https://geode.apache.org/docs/"
  insight="12306 把余票这类高频读写的库存矩阵放进内存数据网格(IMDG),读写分离 + 分区复制,避开数据库行锁热点;下单洪峰再用排队削峰,把瞬时百万请求整流成数据库能消化的稳定速率。"
  application="架构启示:秒杀的本质不是「更快的数据库」,而是把热点数据移出数据库(内存网格)+ 把突发流量整流(排队),两招都在给数据库减负。"
/>
...
- [Apache Geode 官方文档（Pivotal/VMware GemFire 的开源核心，内存数据网格 IMDG）](https://geode.apache.org/docs/) —— 12306 余票查询读写分离所用内存网格技术的一手来源。
