...
<ResearchNote
  title="层级委托:一个没有中心的全球目录"
  source="Mockapetris (1987), RFC 1034: Domain Names - Concepts and Facilities"
  href="https://datatracker.ietf.org/doc/html/rfc1034"
  insight="DNS 把全球命名空间按层级(根 → TLD → 权威)委托出去,配合缓存 TTL,让一个没有中央数据库的系统承载全球查询——每一级只需知道下一跳问谁。"
  application="架构启示:超大规模目录靠「层级委托 + 缓存」而非中心库——把权威分散、把热点用 TTL 缓存吸收,是一切分布式命名(服务发现)的原型。"
/>
