...
<ResearchNote
  ...
  application="在设计代码级 RAG 时，不可单纯使用向量检索。必须在客户端提取 AST 符号树，建立基于符号引用与关键字（BM25）的复合关联，再利用 RRF（Reciprocal Rank Fusion，互惠排名融合：把向量检索与 BM25 字面量检索等多个检索源各自输出的候选列表按排名分值加权合并，完成统一重排的无监督算法）进行重排合并。"
/>
