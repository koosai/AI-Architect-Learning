...
<ResearchNote
  title="V8 isolate:比容器轻两个数量级的隔离"
  source="Cloudflare, Introducing Cloudflare Workers"
  href="https://blog.cloudflare.com/introducing-cloudflare-workers/"
  insight="Cloudflare Workers 用 V8 isolate 而非容器/VM 做多租户隔离:单进程内数千 isolate 共享运行时,冷启动 <5ms、内存开销极小。"
  application="架构启示:隔离粒度决定冷启动与部署密度——当租户代码同语言(JS/Wasm)时,isolate 比容器轻两个数量级,是边缘计算的关键取舍。"
/>
