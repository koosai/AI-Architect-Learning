...
假定发送 2 个数据包 $P_1, P_2$，生成 1 个 FEC 冗余包 $F_{12} = P_1 \oplus P_2$：

$$\text{若 } P_1 \text{ 在网络中丢失，则：} P_1 = P_2 \oplus F_{12}$$

Zoom 根据动态检测到的网络丢包率（Packet Loss Rate），实时调整冗余度 $R = \frac{M}{N}$：网络良好时 $R=0\%$，丢包 30% 时自动提升至 $R=50\%$。
...
<CompareSlider
  before={{
    title: '早期 MCU 硬解码混流架构 (WebEx 时代)',
    description: '服务器 CPU 被编解码拉满，100 人会议需占用多台昂贵硬件服务器，弱网极易卡顿花屏。'
  }}
  after={{
    title: 'Zoom 全球级联 SFU + SVC + 动态 FEC 架构',
    description: '服务器仅做内存包路由，单节点支撑数万人，音视频支持 50% 丢包抗性与 150ms 超低延迟。'
  }}
/>
