...
graph TD
  ...
  Note_NetworkPartition["此时两边 Eureka 依然支持读写，数据最终一致！"]
  NetworkPartition -.-> Note_NetworkPartition
```
...
<ResearchNote
  title="混沌工程:主动注入故障来证明系统能扛"
  source="Netflix 技术博客"
  href="https://netflixtechblog.com/"
  insight="Netflix 用 Chaos Monkey 在生产随机杀实例,逼工程师默认「一切都会挂」而设计容错(Hystrix 熔断、Eureka 服务发现、Zuul 网关);内容分发自建 Open Connect,把缓存服务器塞进 ISP 机房,让热门剧就近吐流。"
  application="架构启示:容错不能靠祈祷要靠演练——主动、常态地注入故障,让韧性成为被持续验证的属性而非纸面设计;带宽密集业务把 CDN 下沉到 ISP 内是省骨干网的杀手锏。"
/>
