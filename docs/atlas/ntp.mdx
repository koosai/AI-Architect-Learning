...
假设网络去程延迟与回程延迟对称（`a = b = delta / 2`），可列出等式：
...
立得 **Offset 偏移量 theta** 与 **RTT 往返延迟 delta**：
...
- **Stepping (跳跃)**：直接修改系统时间（`settimeofday`）。仅在时间偏差巨大（如 `> 128ms`）且开机初始化时使用。
- **Slewing (微调)**：调整 Linux `adjtime`，每秒微调最多 `500 PPM` (0.5ms/s)。如果偏差 10ms，系统将在 20 秒内平滑追平，**绝不会发生时间倒流**。
...
时间戳采用 64 位定点数（32 位整数秒 + 32 位小数秒），分辨率达到 232 皮秒（2^-32 秒）。
...
<CompareSlider
  before={
    <div style={{ padding: '20px', background: '#ffebee', borderRadius: '8px', height: '100%' }}>
      <h3>早期单机 RTC + 简单的 ntpdate 硬跳跃</h3>
      <p>时间跳跃导致数据库 Log 顺序倒置，数据库事务崩溃，虚拟机休眠唤醒后偏差数分钟。</p>
    </div>
  }
  after={
    <div style={{ padding: '20px', background: '#e8f5e9', borderRadius: '8px', height: '100%' }}>
      <h3>Stratum 多源 + Chrony 渐进微调 (Slewing)</h3>
      <p>时钟绝对单调递增，毫秒级微调，云原生 VM 休眠唤醒后 3 秒内恢复微秒级同步。</p>
    </div>
  }
/>
...
<TradeoffExplorer
  ...
  options={[
    ...
    },
    {
      ...
      scores: [1, 5, 2, 5],
      note: '精度达到微秒级（控制在 1ms 以内），可用于分布式 SQL 外部一致性；但机柜硬件成本极其高昂。',
...
- [RFC 5905: Network Time Protocol Version 4 — Protocol and Algorithms Specification (IETF)](https://datatracker.ietf.org/doc/html/rfc5905) —— NTPv4 的一手标准。
- [RFC 5906: Network Time Protocol Version 4 — Autokey Specification (IETF)](https://datatracker.ietf.org/doc/html/rfc5906) —— NTP 安全认证一手标准。
