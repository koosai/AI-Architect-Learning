...
为了完整支撑全球数百万非标房源的搜索、推荐与交易撮合，Airbnb 系统划分为三大核心能力板块：**架构收敛演进**、**非标房源搜推引擎** 与 **日历分布式交易一致性**。
...
Airbnb 系统采用 **Service Blocks 强内聚模块化** 的服务拓扑结构。整体分为 GraphQL 统一网关层、Service Blocks 业务域、搜推与定价 AI 平面以及底层悲观锁/分库存储底座：

### 3a. System Context (系统上下文)

Airbnb 系统的顶层上下文描述了移动终端房客、GraphQL 统一网关、Service Blocks 业务服务块与 Elasticsearch + Embedding 搜推引擎之间的交互关系：
...
在容器级视图中，GraphQL 统一网关将流量智能路由至对应的 Service Blocks（Listing Block、Booking Block、User Block），搜索与 AI 引擎负责实时地理索引召回与个性化 Embedding 重排，存储底座通过 Redis Redlock 实现日历悲观排他锁：
...
<ResearchNote
  title="用 Embedding 把搜索变成个性化召回"
  source="Real-time Personalization using Embeddings(KDD 2018)"
  href="https://dl.acm.org/doi/10.1145/3219819.3219885"
  insight="Airbnb 把房源和用户会话训练进同一向量空间,点击/预订序列定义「相似」,于是搜索排序不再只看关键词匹配,而是「与你此刻意图相近的房源」;冷启动房源靠属性 embedding 兜底。"
  application="架构启示:搜索排序的护城河在信号工程——把用户行为序列变成 embedding,让「相关」由真实转化数据定义,而非人工规则。"
/>
