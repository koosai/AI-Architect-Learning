...
<ResearchNote
  ...
  insight="Megatron-LM 把单层的大矩阵按行/列切到多卡,仅在必要处做 All-Reduce,使大到单卡放不下的层也能训练。"
  application="架构启示:3D 并行(数据/张量/流水线)的选择取决于通信与显存瓶颈——张量并行省显存但通信密集,适合 NVLink 高带宽的机内。"
/>
