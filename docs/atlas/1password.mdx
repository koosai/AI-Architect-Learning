...
<ResearchNote
  title="双钥匙派生:服务器永远拿不到明文密钥"
  source="1Password Security Design Whitepaper"
  href="https://1password.com/security/"
  insight="1Password 用「主密码(你脑中)+ Secret Key(你设备上)」两把钥匙在本地派生加密密钥,服务器只存密文;登录用 SRP 做零知识证明,连密码 hash 都不上传——服务器被拖库也解不开保险库。"
  application="架构启示:真正的零知识不是「加密后上传」,而是让密钥永远不离开客户端;服务端只当密文的哑存储,信任边界收缩到用户设备。"
/>
