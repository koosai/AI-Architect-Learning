...
<CompareSlider
  ...
  after={
    <div style={{ padding: '20px', background: '#e8f5e9', border: '1px solid #2e7d32', borderRadius: '8px', height: '100%', color: '#333' }}>
      ...
      <p style={{ marginTop: '10px', fontSize: '0.9rem' }}>采用磁盘顺序写日志，消息在保留期内永不删除。采用 Pull（拉取）模型，消费者带着 offset 自主去 Broker 批量拉数据，Broker 属于纯无状态的消息路由器。优点是吞吐高达百万级，支持重复重放消费与完美的历史积压能力；缺点是无法支持复杂的消息优先路由。</p>
    </div>
  }
