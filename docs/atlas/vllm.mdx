...
graph TD
  subgraph SchedulingComparison ["传统 Batching 与 Continuous Batching 调度对比')"]
    subgraph StaticBatching ["静态 Batching (请求级阻塞等待)')"]
      ...
      Note_SB_Iter2["Req_2 已经完成, 但由于 Req_1 没完,<br/>必须在 Batch 里无意义占用显存和计算！"]
      SB_Iter2 -.-> Note_SB_Iter2
    end
    ...
    subgraph ContinuousBatching ["连续 Batching (迭代级即刻插入与退出)')"]
      ...
      Note_CB_Iter2["新请求即时插入, 完成请求即刻退出,<br/>GPU 始终饱和工作！"]
      CB_Iter2 -.-> Note_CB_Iter2
    end
  end
```
...
<ResearchNote
  title="PagedAttention:给 KV Cache 做虚拟内存分页"
  source="PagedAttention 论文(SOSP 2023)"
  href="https://arxiv.org/abs/2309.06180"
  insight="LLM 推理的 KV cache 长度不定、易碎片化浪费显存;vLLM 借操作系统分页思想,把 KV cache 切成固定小块非连续存储,按需分配、多请求共享前缀块,显存利用率逼近满载;配合连续批处理,一个序列结束就立刻插入新请求。"
  application="架构启示:把成熟的 OS 抽象(分页/虚拟内存)迁移到新领域常有奇效——KV cache 的碎片问题本质同物理内存碎片,分页 + 按需分配是同一副解药。"
/>
