...
<ResearchNote
  title="网络所有权:谁模拟这个物体,谁说了算"
  source="Roblox 开发者文档"
  href="https://create.roblox.com/docs"
  insight="Roblox 让每个物理物体有一个「网络所有者」(通常是最近的玩家客户端),由它本地模拟物理再把结果复制给他人,把物理算力分摊到客户端;UGC 脚本跑在 Luau 沙箱里,受限 API + 类型系统防止恶意代码越界。"
  application="架构启示:海量 UGC 平台要把算力和信任都「下沉又设界」——物理下放客户端省服务器,但用网络所有权定权威;代码开放给用户,但用沙箱语言划死边界。"
/>

## 来源

:::caution 关于来源与置信度
本案例的具体实现细节为**基于公开信息的教学重构**，非官方架构文档。Luau 语言与脚本沙箱、DataModel 实例树、网络所有权（Network Ownership）等概念有官方公开资料佐证；分布式物理与刚体求解的内部实现请以官方为准。下列为可延伸查证的公开资料方向（按名称检索）：
:::

- [Luau](https://luau-lang.org/) —— Roblox 开源的脚本语言（沙箱、类型系统）官方站。
- [Roblox 官方开发者文档（create.roblox.com/docs）](https://create.roblox.com/docs) —— DataModel 实例树、网络所有权（Network Ownership）与复制机制的一手文档。
- 多线程游戏引擎中投影高斯-赛德尔（PGS）刚体求解的通用原理 —— 见公开的实时物理仿真资料。