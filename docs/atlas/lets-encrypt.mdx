...
<PyRunner
  expect="ACME HTTP-01校验与90天证书轮转验证成功"
  rows={25}
  code={`import time
...
print("\\n[Result] ACME HTTP-01校验与90天证书轮转验证成功")
`}
/>
...
<ResearchNote
  title="ACME:把 HTTPS 变成默认值"
  source="Let us Encrypt Documentation"
  href="https://letsencrypt.org/docs/"
  insight="Let us Encrypt 用 ACME 协议自动化「证明域名控制权 → 签发 → 续期」全流程,配合 90 天短生命周期证书,把 HTTPS 从手动、昂贵、易过期变成免费、自动、默认。"
  application="架构启示:让正确的事变成默认——用自动化 + 短生命周期(强制轮转)消除人工运维,既提安全又降负担(同思想见于密钥轮转、令牌短 TTL)。"
/>
