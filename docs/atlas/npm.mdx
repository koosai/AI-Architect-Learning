...
graph TB
  ...
  subgraph CloudRegistry["Cloud Infrastructure"]
    CDN POP["Fastly Edge CDN POP (全球边缘节点)"]
    StorageS3["Tarball Storage (S3 / GCS)"]
    MasterDB["PostgreSQL / CouchDB (包元数据主库)"]
  end

  PackageCLI -->|1. 校验 Lockfile 并查询元数据| CDN POP
  CDN POP -.->|2. 未命中时源站取回| MasterDB
  CDN POP -.->|3. 读取物理 .tgz| StorageS3
  ...
  style CDN POP fill:#bbdefb,stroke:#1976d2
```
  ...
  questions={[
    {
      ...
      answer: 1,
      explain: '读写分离与全球边缘 CDN 是 npm 支撑万亿级流量的精髓。源站专注于写与元数据变更，99.9% 的只读下载流量被 Fastly CDN POP 节点在边缘直接拦截消化。'
    },
    {
