...
:::caution 关于来源
以下为 Shopify 官方工程博客（Shopify Engineering）公开分享的文章标题，可按名称检索原文。文中的秒杀防超卖等具体数值为**教学示意**，非官方参数。
:::

- Shopify Engineering —— 《Deconstructing the Monolith》：从 Rails 单体拆分与模块化边界（Pods 多租户架构的背景）。
- Shopify Engineering —— 《Sorting Hat》：如何把海量请求路由到相互独立的 Pod（请求分片路由）。
