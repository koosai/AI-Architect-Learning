...
mindmap
  root((SQLite 架构))
    ...
    存储与缓存平面
      ...
      <GlossaryTerm term="Rollback Journal (回滚日志)" definition="Rollback Journal：SQLite 传统的事务原子性保证机制。在修改磁盘页前，先将原物理页的旧数据拷贝备份至 .journal 文件中。若事务失败或崩溃，通过将 .journal 中的旧页覆盖回原 DB 文件实现回滚。但在写入期间会阻塞读操作。">Rollback Journal</GlossaryTerm>
      <GlossaryTerm term="WAL (预写日志模式)" definition="WAL (Write-Ahead Logging)：SQLite 现代高并发事务模式。所有写操作顺序追加写入独立的 .wal 文件中，原 DB 文件保持只读。使得读事务与写事务互不阻塞，大大提升了多线程并发吞吐。">WAL 预写日志模式</GlossaryTerm>
    OS 操作系统移植层
      <GlossaryTerm term="VFS (虚拟文件系统)" definition="VFS (Virtual File System)：SQLite 的操作系统接口抽象层。它将物理文件的打开、关闭、读写、加锁及 sync 落盘抽象为标准 C 结构体句柄，使 SQLite 能够轻松移植到 Windows、POSIX、FreeRTOS、VxWorks 等各种 OS 上。">VFS 抽象层</GlossaryTerm>
      共享内存 (.shm 校验索引)
```
