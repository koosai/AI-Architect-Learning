...
graph TD
    ...
    Note_Step2["端到端不通过，通过分步 Trace 准确归因病灶为 Analyst Agent<br/>拒绝盲目重写全局 Proposer prompt，定点优化"]
    Step2 -.-> Note_Step2
```
...
graph TD
    ...
    Note_Gate["拦截所有跨 Agent 的高危副作用调用，<br/>不能仅依赖单个 Agent 的局部 System Prompt 拦截"]
    Gate -.-> Note_Gate
```
