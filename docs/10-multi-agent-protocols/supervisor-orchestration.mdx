...
<GlossaryTerm term="Supervisor Pattern" definition="主管模式：一个主控（supervisor）Agent 负责编排——把任务分配给合适的专精（worker）Agent、收集它们的结果、决定下一步、并作为唯一的最终出口。是最常用、最可控的多 Agent 协作结构。">Supervisor（主管）模式</GlossaryTerm>：一个主控 Agent 负责编排，专精 Agent 是它的“下属”（worker）：
...
Supervisor 模式之所以是多 Agent 的首选，因为它**集中了控制**（[呼应 M5L2 网关收口](../core-components/api-gateway)）：
...
- **分配可以是确定的或自主的**：简单场景用**固定流程**（workflow 编排：研究→设计→审查，[呼应 M9L11/M10L1 优先 workflow](./why-multi-agent)）；复杂场景才让 Supervisor **自主决定**派给谁做什么。前者更可控，后者更灵活——按需选。
- **结构化的任务和结果**：Supervisor 派任务、worker 回结果，都要用 [schema（M7L6/M10L4）](./communication-state)，而非自由文本——保证协作可靠。
