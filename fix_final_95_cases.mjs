...
function generateFixCandidates(code) {
  ...
  // Candidate 1: Targeted line transformations with sequence note arrow fixes
  const lines1 = [];
  for (let l of lines) {
    ...
    if (diagType === 'sequenceDiagram') {
      ...
      } else if (st.startsWith('Note over') || st.startsWith('Note left of') || st.startsWith('Note right of')) {
        line = line.replace(/->/g, '→');
      }
      lines1.push(line);
      continue;
  ...
  // Candidate 2: Clean node string contents aggressively
  const lines2 = [];
  for (let l of lines1) {
    let line = l;
    if (diagType === 'flowchart') {
