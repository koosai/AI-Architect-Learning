...
function generateFixCandidates(code) {
  ...
  // Candidate 1: Targeted line transformations with sequence note arrow fixes & decision pipe fixes
  const lines1 = [];
  for (let l of lines) {
    ...
    if (diagType === 'flowchart' || firstLine.includes('graph')) {
      ...
      // Fix decision nodes with inner pipe symbols: CheckTolerance{...|ratio|...}
      if (line.includes('{') && line.includes('}')) {
        line = line.replace(/(\b[A-Za-z0-9_\-\.]+\s*\{)([^}]+)(\})/g, (m, p1, inner, p3) => {
          let clean = inner.replace(/\|([^|\n]+)\|/g, 'abs($1)');
          return `${p1}${clean}${p3}`;
        });
      }

      // Restore Stadium Shape: Start("[Text")]) -> Start(["Text"])
  ...
  // Candidate 2: Aggressive sanitization of node labels
  const lines2 = [];
  for (let l of lines1) {
    let line = l;
    if (diagType === 'flowchart' || firstLine.includes('graph')) {
